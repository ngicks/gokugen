...
func BenchmarkQueue_RemoveCancelled_1000_allCancelled(b *testing.B) {
	...
	now := time.Now()
	for i := 0; i < 1000; i++ {
		t := scheduler.NewTask(now, func(scheduled time.Time) {})
		t.Cancel()
		q.Push(t)
	}
	q.Push(scheduler.NewTask(now.Add(-1*time.Second), func(scheduled time.Time) {}))
...
func BenchmarkQueue_RemoveCancelled_100000_oneCancelled(b *testing.B) {
	...
	t := scheduler.NewTask(now.Add(-1*time.Second), func(scheduled time.Time) {})
	t.Cancel()
	q.Push(t)
