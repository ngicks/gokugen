...
func testRepository_update_to_known_value(t *testing.T, repo def.Repository, debug bool) {
	...
	require.True(option.Equal(updateParam.Deadline.Value(), updated.Deadline))
	require.True(option.Equal(task.CancelledAt, updated.CancelledAt))
	require.True(option.Equal(task.DispatchedAt, updated.DispatchedAt))
	require.True(option.Equal(task.DoneAt, updated.DoneAt))
