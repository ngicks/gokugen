...
func testRepository_update_to_known_value(t *testing.T, repo def.Repository, debug bool) {
	...
	require.True(updateParam.Deadline.Value().EqualFunc(updated.Deadline, time.Time.Equal))
	require.True(task.CancelledAt.EqualFunc(updated.CancelledAt, time.Time.Equal))
	require.True(task.DispatchedAt.EqualFunc(updated.DispatchedAt, time.Time.Equal))
	require.True(task.DoneAt.EqualFunc(updated.DoneAt, time.Time.Equal))
